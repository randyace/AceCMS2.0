...
export function CMSSidebar({ activeSection }: SidebarProps) {
  ...
  const [expandedItems, setExpandedItems] = useState<Record<string, boolean>>({ 'services-group': true });
  ...
  const resolvedExpandedItems = useMemo(() => {
    // Keep parent section visible whenever one of its children is active.
                ...
                <ul className="mt-0.5 space-y-0.5">
                  {group.items.map((item) => {
                    ...
                    if (item.children && item.children.length > 0) {
                      const isExpanded = resolvedExpandedItems[item.id] ?? false;
