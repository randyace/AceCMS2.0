...
function generateOrderPDF(order: WebOrder) {
  ...
  printWindow.document.write(`
    ...
    <head>
      ...
      <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .header { background: #115160; color: white; padding: 20px; border-radius: 8px; margin-bottom: 20px; }
        ...
        .po-section h3 { color: #115160; margin-bottom: 10px; }
        .po-tag { display: inline-block; background: #115160; color: white; padding: 3px 10px; border-radius: 4px; margin: 3px; font-size: 11px; }
    ...
    <body>
      <div class="header">
        <div style="display:flex; justify-content:space-between; align-items:center;">
          ...
          <div style="text-align:right; font-size:11px; opacity:0.8;">
            <p>ShopAdmin CMS</p><p>admin@shopco.com</p>
          </div>
        </div>
      </div>
      <div class="grid">
      ...
      <div class="footer"><p>Generated by ShopAdmin CMS &nbsp;|&nbsp; ${new Date().toLocaleDateString()}</p></div>
        ...
        <div className="bg-gradient-to-r from-[#115160] to-[#1a7a8f] text-white px-6 py-5">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
            ...
            <div className="flex gap-2 flex-wrap">
              ...
              <Button onClick={handleSave} className="bg-[#cec18a] text-[#115160] hover:bg-[#d4c990]">Save Changes</Button>
            </div>
          </div>
        </div>
                    ...
                    <React.Fragment key={step}>
                      <div className="flex flex-col items-center gap-1 flex-1 min-w-0">
                        <div className={`w-8 h-8 rounded-full flex items-center justify-center text-xs border-2 transition-colors flex-shrink-0 ${
                          i <= currentStepIndex ? 'bg-[#115160] border-[#115160] text-white' : 'bg-background border-border text-muted-foreground'
                        }`}>
                      ...
                      {i < STATUS_FLOW.length - 1 && (
                        <div className={`flex-1 h-0.5 mb-5 sm:mb-4 transition-colors ${i < currentStepIndex ? 'bg-[#115160]' : 'bg-border'}`} />
                      )}
                    </React.Fragment>
                  ))}
                </div>
            ...
            <Card className="shadow-sm overflow-hidden">
              ...
              <CardContent className="space-y-2 text-sm p-5">
                ...
                <div className="flex justify-between"><span className="text-muted-foreground">Customer</span>
                  <button onClick={() => navigateTo('members', editingOrder.customerId)} className="text-[#115160] hover:underline flex items-center gap-1">
                  ...
                  <tbody>
                    {editingOrder.items.map((item, i) => (
                      <tr key={i} className="border-b border-border last:border-0 hover:bg-muted/20">
                        <td className="px-4 py-2.5 font-mono text-xs text-[#115160]">{item.sku}</td>
                    ...
                    <div key={rel.po} className="flex items-center justify-between p-3 bg-amber-50/50 border border-amber-100 rounded-lg">
                      <div>
                        <button
                          ...
                          className="text-[#115160] hover:underline font-mono text-sm font-medium flex items-center gap-1"
                        >
                      ...
                      <Button
                        size="sm"
                        variant="outline"
                        className="text-xs border-[#115160]/30 text-[#115160] hover:bg-[#115160]/5"
                        onClick={() => navigateTo('purchase-orders', rel.po)}
                      >
                        View PO
                      </Button>
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-sm min-w-[600px]">
                <thead className="border-b border-border bg-[#115160]/5">
                  <tr>
                    <th className="text-left px-4 py-3 text-[#115160] text-xs">Order #</th>
                    <th className="text-left px-4 py-3 text-[#115160] text-xs">Customer</th>
                    <th className="text-right px-4 py-3 text-[#115160] text-xs">Amount</th>
                    <th className="text-left px-4 py-3 text-[#115160] text-xs">Status</th>
                    <th className="text-left px-4 py-3 text-[#115160] text-xs">Date</th>
                ...
                <tbody>
                  {filtered.map((o) => (
                    <tr key={o.id} className="border-b border-border last:border-0 hover:bg-[#115160]/5 transition-colors">
                      <td className="px-4 py-3 text-[#115160] font-mono text-xs font-medium">{o.orderNumber}</td>
                      ...
                      <td className="px-4 py-3 text-right">
                        <div className="flex items-center justify-end gap-1">
                          <Button variant="ghost" size="sm" onClick={() => openEdit(o)} className="hover:bg-[#115160]/10 hover:text-[#115160] text-xs">View</Button>
